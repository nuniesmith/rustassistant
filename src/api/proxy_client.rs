//! Standalone RustAssistant proxy client for external apps (e.g. the futures trading app).
//!
//! # Purpose
//!
//! Drop this file into any Rust project that currently calls Grok/xAI directly.
//! It wraps the RustAssistant OpenAI-compatible `/v1/chat/completions` endpoint
//! and falls back to the real Grok API transparently when:
//!
//!   - RustAssistant is unreachable (connection refused, timeout, DNS failure)
//!   - RustAssistant returns a 5xx error
...
//! println!("answered by: {}", reply.answered_by);  // "rustassistant" | "grok"
...
//! |---------------------------|---------------------------------|--------------------------------------------|
//! | `RA_BASE_URL`             | `http://localhost:3500`         | RustAssistant server URL                   |
//! | `RA_API_KEY`              | *(empty — open endpoint)*       | Bearer token for RustAssistant             |
//! | `RA_TIMEOUT_SECS`         | `15`                            | Per-request timeout before fallback fires  |
//! | `RA_MODEL`                | `auto`                          | Model hint sent to RustAssistant           |
//! | `RA_REPO_ID`              | *(none)*                        | Repo context to inject (optional)          |
//! | `RA_FORCE_REMOTE`         | `false`                         | Force Grok even when calling RA            |
...
//! | `PROXY_CLIENT_DISABLE_RA` | `false`                         | Skip RA entirely, always use Grok directly |
...
pub enum ProxyClientError {
    #[error("Both RustAssistant and Grok fallback failed: RA={ra}, Grok={grok}")]
    BothFailed { ra: String, grok: String },

    #[error(
        "Grok fallback is not configured (XAI_API_KEY is unset) and RustAssistant failed: {ra}"
    )]
    FallbackUnconfigured { ra: String },
...
pub struct ProxyClientConfig {
    // ── RustAssistant ────────────────────────────────────────────────────────
    /// Full base URL of the RustAssistant server, e.g. `http://10.0.1.5:3500`.
    pub ra_base_url: String,

    /// Bearer token for RustAssistant (`RA_API_KEY`).
    /// Empty string → no `Authorization` header sent (open endpoint).
    pub ra_api_key: String,

    /// Per-request timeout when calling RustAssistant.
    ...
    /// Force remote (Grok) inside RustAssistant regardless of task kind.
    pub ra_force_remote: bool,

    /// When `true`, skip RustAssistant entirely and call Grok directly.
...
impl ProxyClientConfig {
    ...
    pub fn from_env() -> Self {
        let ra_timeout = std::env::var("RA_TIMEOUT_SECS")
            .ok()
        ...
        let ra_force_remote = std::env::var("RA_FORCE_REMOTE")
            .map(|v| v.eq_ignore_ascii_case("true") || v == "1")
            .unwrap_or(false);

        Self {
            ra_base_url: std::env::var("RA_BASE_URL")
                .unwrap_or_else(|_| "http://localhost:3500".to_string()),
            ra_api_key: std::env::var("RA_API_KEY").unwrap_or_default(),
            ra_timeout: Duration::from_secs(ra_timeout),
            ra_model: std::env::var("RA_MODEL").unwrap_or_else(|_| "auto".to_string()),
            ra_repo_id: std::env::var("RA_REPO_ID").ok().filter(|v| !v.is_empty()),
            ra_force_remote,
            disable_ra,
...
impl<'c> ChatRequestBuilder<'c> {
    ...
    /// Force Grok inside RustAssistant for this request.
...
pub struct ChatReply {
    ...
    /// Which backend actually answered: `"rustassistant"` or `"grok"`.
    ...
    /// Prompt tokens (approximate when RA answered).
    pub prompt_tokens: u32,

    /// Completion tokens (approximate when RA answered).
    pub completion_tokens: u32,

    /// True when the local Ollama model was tried but fell back to Grok
    /// inside RustAssistant.
    pub used_internal_fallback: bool,

    /// True when the response was served from RustAssistant's cache.
    pub cached: bool,

    /// The `TaskKind` RustAssistant assigned (empty string when Grok answered
    /// directly).
...
/// RustAssistant proxy client with transparent Grok fallback.
///
...
impl ProxyClient {
    ...
    pub fn new(config: ProxyClientConfig) -> Self {
        ...
        let http = Client::builder()
            ...
            .user_agent("rustassistant-proxy-client/1.0")
            .build()
    ...
    /// Build from environment variables but override the RA base URL.
    ...
    /// Probe RustAssistant with a `GET /health` request.
    ///
    ...
    async fn chat_inner(
        ...
        // When RA is disabled, skip straight to Grok.
        if !self.config.disable_ra {
            match self
                .try_rustassistant(
                    &messages,
                ...
                .await
            {
                ...
                Err(ra_err) => {
                    // Log the RA failure and fall through to Grok.
                    eprintln!(
                        "[proxy_client] RustAssistant unavailable or errored — \
                         falling back to Grok. RA error: {}",
                        ra_err
                    );
        ...
        // RA disabled — call Grok directly.
        self.try_grok(
            &messages,
            temperature,
            max_tokens,
            "RA disabled".to_string(),
        )
        .await
    }
    ...
    async fn try_rustassistant(
        ...
        if status.is_server_error() {
            ...
            return Err(format!("RA server error {}: {}", status, text));
        }
        ...
        if !status.is_success() {
            let text = http_resp
                ...
                .await
                .unwrap_or_else(|_| "<unreadable body>".to_string());
            return Err(format!("RA client error {}: {}", status, text));
        }

        let oai: OaiResponse = http_resp
            .json()
            .await
            .map_err(|e| format!("Failed to parse RA response: {}", e))?;
        ...
        Ok(ChatReply {
            content,
            answered_by: "rustassistant".to_string(),
    ...
    async fn try_grok(
        ...
        let api_key = match &self.config.xai_api_key {
            Some(k) => k.clone(),
            None => {
                return Err(ProxyClientError::FallbackUnconfigured { ra: ra_err_str });
            }
        };
...
impl ChatReply {
    /// True when RustAssistant (local Ollama or internal Grok) answered.
    pub fn from_rustassistant(&self) -> bool {
        self.answered_by == "rustassistant"
    }

    /// True when the upstream Grok API answered (RA was unavailable).
    ...
    /// when RA answered with the local model the cost is effectively $0.
    pub fn estimated_cost_usd(&self) -> f64 {
        if self.from_rustassistant() && !self.used_internal_fallback {
...
mod tests {
    ...
    fn chat_reply_cost_estimate_zero_for_local() {
        ...
        assert!(reply.from_rustassistant());
    ...
    fn chat_reply_cost_estimate_nonzero_for_internal_fallback() {
        // RA answered but routed internally to Grok — still has a cost.
        let reply = ChatReply {
