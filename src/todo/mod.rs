...
//!
//! ```text
//! rustassistant todo-scan     <repo-path>   [--json] [--filter high|medium|low]
//! rustassistant todo-scaffold <repo-path>   [--dry-run] [--overwrite] [--output scaffold.json]
//! rustassistant todo-plan     <todo-md>     [--context <dir>] [--output gameplan.json]
//! rustassistant todo-work     <batch-json>  [--dry-run]
//! rustassistant todo-sync     <todo-md>     <results-json> [--dry-run] [--append-summary]
//! ```
//!
//! # Testing against rustassistant itself
//!
//! The pipeline is designed to be self-hosting: running it against the
//! `rustassistant` repo is the canonical integration test.  All repos managed
//! by RustAssistant share the same project layout conventions, so the same
