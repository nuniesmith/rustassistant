...
pub async fn ideas_handler(
    ...
    // Tag filter buttons
    let tag_filter_html: String = tags
        .iter()
        .take(15)
        .map(|t| {
...
pub async fn add_idea_handler(
    ...
    let pool = &state.db.pool;
    let priority = form.priority.unwrap_or(3) as i64;
    ...
    .await
    {
        Ok(_) => {
            info!(
                "Created idea: {}",
                &form.content[..form.content.len().min(50)]
            );
        }
        Err(e) => {
...
pub async fn docs_handler(
    ...
    let documents = if let Some(ref q) = params.q {
        search_documents(pool, q).await.unwrap_or_default()
    } else {
        list_documents(
            pool,
            params.doc_type.clone(),
            None, // repo_id
            Some(50),
            None, // offset
        )
    ...
    let type_tabs: String = doc_types
        .iter()
        .map(|dt| {
            ...
            format!(
                r#"<a href="/docs?doc_type={}" class="{}">{}</a>"#,
                dt, cls, dt
            )
        })
        .collect::<Vec<_>>()
...
pub async fn create_doc_handler(
    ...
    let tags = form.tags.as_ref().map(|t| {
        t.split(',')
    ...
    match get_document(pool, &id).await {
        Ok(doc) => {
            let tags_html = doc
                ...
                .map(|t| {
                    t.split(',')
                        .map(|tag| format!(r#"<span class="tag">{}</span>"#, tag.trim()))
                        .collect::<Vec<_>>()
                        .join(" ")
                })
                .unwrap_or_default();
...
pub async fn activity_handler(State(state): State<Arc<WebAppState>>) -> impl IntoResponse {
    let pool = &state.db.pool;
        ...
        events
            ...
            .map(|e| {
                ...
                let details_html = e
                    .details
                    .as_deref()
                    .map(|d| format!(r#"<div class="event-details">{}</div>"#, html_escape(d)))
                    .unwrap_or_default();
...
pub async fn activity_feed_partial(State(state): State<Arc<WebAppState>>) -> impl IntoResponse {
    let pool = &state.db.pool;
    let events = get_recent_events(pool, 100, None).await.unwrap_or_default();
...
pub async fn repo_settings_handler(
    ...
    // Get current repo
    let repo =
        sqlx::query_as::<_, crate::db::Repository>("SELECT * FROM repositories WHERE id = ?1")
            .bind(&id)
            .fetch_optional(pool)
            .await;
...
pub async fn scan_progress_api(State(state): State<Arc<WebAppState>>) -> impl IntoResponse {
    let pool = &state.db.pool;

    let repos: Vec<RepoScanStatus> = sqlx::query_as::<
        _,
        (
            String,
            String,
