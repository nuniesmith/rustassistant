...
impl ApiError {
    fn not_found(msg: impl Into<String>) -> (StatusCode, Json<Self>) {
        (
            ...
            Json(Self { error: msg.into(), code: "NOT_FOUND".to_string() }),
        )
    }
    ...
    fn internal(msg: impl Into<String>) -> (StatusCode, Json<Self>) {
        (
            ...
            Json(Self { error: msg.into(), code: "INTERNAL_ERROR".to_string() }),
        )
    }
}
    ...
    F: FnOnce(&RegisteredRepo) -> std::path::PathBuf,
{
    ...
    let repo = match service.get_repo(repo_id) {
        Some(r) => r,
        None => return ApiError::not_found(format!("Repo '{}' not found", repo_id)).into_response(),
    };
